...
-- CREATE INDEX IF NOT EXISTS idx_stg_flights_grid ON staging.stg_flights (grid_lat, grid_lon);
...
country_airline AS (
    SELECT DISTINCT ON (dc.origin_country)
        dc.origin_country       AS country,
        a.airline_name,
        a.iata_code             AS airline_iata,
        a.icao_code             AS airline_icao
    FROM distinct_countries dc
    JOIN staging.stg_airlines a
        ON dc.origin_country = a.country
        AND a.is_active = TRUE
    ORDER BY
        dc.origin_country,
        a.airline_name ASC      -- desempate determinístico sin COUNT
)
